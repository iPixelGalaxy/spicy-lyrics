...
import { ANIMATOR_PARAMETERS, $animatorDetail, $animatorValues, $savedAnimatorPresets, $selectedAnimatorPresetId, deleteSavedAnimatorPreset, importAnimatorPreset, loadSavedAnimatorPreset, randomizeAnimatorValues, saveAnimatorPreset, shareAnimatorPreset, type AnimatorDetail, resetAnimatorValues, setAnimatorValue } from "../../../utils/Lyrics/Animator/Tuning.ts";
...
export default function AnimatorBehaviorPanel({ onBack, onClose, eventDocument = document }: { onBack: () => void; onClose: () => void; eventDocument?: Document }) {
  ...
  const presetToShare = presets.find((preset) => preset.id === presetToLoad) ?? selectedPreset;
  ...
  return <div className="slm sl-sp-panel" style={{ padding: "8px 0" }}>
    ...
    <div className="sl-sp-preset-actions"><input className="sl-sp-text-input" value={name} onChange={(event) => setName(event.currentTarget.value)} placeholder="Preset name" aria-label="Preset name" /><button className="sl-sp-btn" type="button" disabled={!name.trim()} onClick={() => { if (saveAnimatorPreset(name)) setName(""); }}>Save</button><button className="sl-sp-btn" type="button" disabled={!presetToLoad} onClick={() => { deleteSavedAnimatorPreset(presetToLoad); setPresetToLoad(""); }}>Delete Current</button><button className="sl-sp-btn" type="button" onClick={async () => { try { const code = shareAnimatorPreset(presetToShare?.name ?? name, presetToShare?.values); const clipboard = Spicetify.Platform?.ClipboardAPI; if (clipboard?.copy) await clipboard.copy(code); else if (navigator.clipboard) await navigator.clipboard.writeText(code); else throw new Error("Clipboard is unavailable."); setMessage("Preset copied to clipboard."); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not copy preset."); } }}>Share</button><button className="sl-sp-btn" type="button" onClick={async () => { try { const clipboard = Spicetify.Platform?.ClipboardAPI; const code = clipboard?.paste ? await clipboard.paste() : await navigator.clipboard?.readText(); if (!code?.trim()) throw new Error("Clipboard has no preset code."); setMessage(`Imported ${importAnimatorPreset(code.trim())}.`); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not import preset."); } }}>Import</button></div>
