...
function capitalizeFirstGeneratedWord(words: GeneratedWord[]) {
  ...
  if (words[0].TransliteratedText !== undefined) {
    words[0].TransliteratedText = capitalizeLeadingLetter(words[0].TransliteratedText);
  }
...
function assignWordText(
  words: GeneratedWord[],
  key: "TransliteratedText" | "GibberishText",
...
function createFallbackLead(
  ...
  assignWordText(leadWords, "TransliteratedText", TransliteratedText);
...
function buildStaticSyntheticLineLyrics(
  ...
  preparedLines.forEach((entry, index) => {
    ...
    syntheticLines.push({
      Type: "Vocal",
      Text: lineText,
      TransliteratedText: entry.line.TransliteratedText,
...
export function ConvertLineLyricsToExperimentalWordSync(
  ...
  for (let lineIndex = 0; lineIndex < sourceLines.length; lineIndex += 1) {
    ...
    const romanizedSegments = getAlignedSegmentTexts(baseSegments, line.TransliteratedText);
    ...
    baseSegments.forEach((segment, index) => {
      ...
      if (romanizedSegments?.[index]) {
        assignWordText(generatedWords, "TransliteratedText", romanizedSegments[index]);
      }
    ...
    if (!romanizedSegments && leadWords.length > 0) {
      assignWordText(leadWords, "TransliteratedText", line.TransliteratedText);
    }
    ...
    const fallbackLead = createFallbackLead(
      ...
      romanizedSegments?.join(" ") ?? line.TransliteratedText,
